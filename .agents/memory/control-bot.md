...
- **Telegram blocks login codes typed as plain digits in any Telegram chat** (anti-phishing: "code was previously shared by your account"). The bot login flow must instruct the user to type the code WITH separators (e.g. `1 3 6 0 5` / `1-3-6-0-5`) and strip non-digits (`text.replace(/\D/g,"")`) before POSTing to `/login/code`. Most reliable alternative: log in via the website dashboard (code entered outside Telegram).
  **Why:** typing the bare code into the bot chat = sending a Telegram message containing the code → Telegram invalidates it and blocks the sign-in.
