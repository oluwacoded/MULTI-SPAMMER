...
**Streaming valid phrases — only emit after full evaluation:** the worker streams each checksum-valid mnemonic to the UI so results survive an early Stop. Emit the `valid` message only AFTER the per-mode check has ruled it out as a hit (target compare done, or — in scan mode — all addresses' `checkActivity` completed with no funds), never before. **Why:** Stop does an immediate `worker.terminate()` that drops in-flight messages; if a phrase is streamed before its on-chain scan finishes, the UI's "passed checksum but did NOT match your on-chain activity — none is your wallet" disclaimer can be false for a phrase that was never fully checked, risking the user discarding the real wallet. A code review failed an earlier version that streamed before the scan loop.
