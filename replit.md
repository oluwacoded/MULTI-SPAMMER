...
- **Runtime config persists in Postgres (survives redeploys):** `writeJSON` in `botInstance.ts` writes each JSON file to disk AND write-throughs to the `app_config` key-value table (`key`=filename, `value`=jsonb) in `@workspace/db`. On boot, `index.ts` calls `restoreConfigToDisk()` (`lib/configStore.ts`) BEFORE creating the engine, hydrating `data/` files from the DB (and seeding the DB from any disk-only files). This is why Telegram logins + `api_id/api_hash` now survive republishes (the `data/` dir itself is still gitignored/ephemeral). WhatsApp `whatsapp-auth/` and `campaign-history/` are dirs written via raw `fs` and are NOT yet DB-backed.
